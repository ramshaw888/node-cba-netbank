...
class API {
  ...
  logon(username, password) {
    ...
    return this.web.get(this.getUrl(PATH.LOGON))
    //  Parse the page to get logon form
    .then(resp => this.refreshBase(resp)).then(parser.parseForm).then(resp =>
  ...
  getTransactionHistory(account, from = moment().subtract(6, 'years').format(moment.formats.default), to = moment().format(moment.formats.default)) {
    ...
    return this.web.get(this.getUrl(account.link)).then(parser.parseForm).then(parser.parseAccountListWithKeys).then(resp => this.refreshBase(resp)).then(resp => {
      ...
      if (!resp.form.ctl00$BodyPlaceHolder$radioSwitchSearchType$field$) {
        return this.lazyLoading(resp, acc).then(r => this.getTransactionsByDate(r, acc, from, to)) //  attach pendings
        .then(r => Object.assign({}, r, { pendings: resp.pendings }));
      }
      return this.getTransactionsByDate(resp, acc, from, to)
      //  attach pending
      .then(r => Object.assign({}, r, { pendings: resp.pendings })).then(r => {
  ...
  lazyLoading(response, account) {
    ...
    return this.web.post({
      ...
      partial: true
    }).then(parser.parseViewState).then(resp => parser.parseForm(resp).catch(() => resp)).then(resp => Object.assign({}, resp, { form: Object.assign({}, response.form, resp.form) }));
  }
  ...
  getMoreTransactions(response, account) {
    ...
    return this.web.post({
      ...
      form,
      partial: true
    }).then(parser.parseTransactions).then(resp => this.refreshBase(resp)).then(resp => {
      if (!resp.more || resp.limit) {
        //  There is no more transactions or reached the limit.
        return resp;
      }
      return this.getMoreTransactions(Object.assign({}, resp, { form }), Object.assign({}, account, { link: Url.parse(resp.url).path }))
  ...
  getTransactionsByDate(response, account, from, to) {
    ...
    return this.web.post({
      ...
      return this.getMoreTransactions(Object.assign({}, resp, { form }), Object.assign({}, account, { link: Url.parse(resp.url).path }))
      //  concat more transactions.
      .then(r => Object.assign({}, r, { form, transactions: concat(resp.transactions, r.transactions) })).then(r => {
        ...
        let earliest = r.transactions[0].timestamp;
        ...
        return this.getTransactionsByDate(Object.assign({}, r, { form }), Object.assign({}, account, { link: Url.parse(r.url).path }), from, newTo)
        //  concat more transactions
        .then(rr => Object.assign({}, rr, { form, transactions: concat(r.transactions, rr.transactions) })).catch(err => {
