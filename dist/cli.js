...
const ui = new UI();
const myArgv = yargs.usage('CBA Netbank CLI\nUsage: $0 <command> [args]').option('u', {
  alias: 'username',
  ...
  describe: 'password',
  type: 'string'
}).command('list', 'List accounts', () => {}, argv => {
  ...
  ui.logon(argv).then(accounts => {
    //  matching accounts
    const account = accounts.find(a => a.name.toLowerCase().indexOf(argv.account.toLowerCase()) >= 0 || a.number.indexOf(argv.account) >= 0);
    if (account) {
      ...
      ui.downloadHistory(account, argv.from, argv.to).then(history => {
        ...
        const filename = argv.output.replace(tagAccountName, account.name).replace(tagAccountNumber, account.number).replace(tagFrom, moment(argv.from, moment.formats.default).format(moment.formats.sortable)).replace(tagTo, moment(argv.to, moment.formats.default).format(moment.formats.sortable)).replace(tagExt, argv.format);
