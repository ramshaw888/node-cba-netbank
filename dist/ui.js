...
class UI {
  ...
  start(argv) {
    ...
    }).then(c => this.logon(c)).then(() => this.chooseAccountAndShowHistory(argv.months)).catch(error => {
      debug(error);
      console.log('Bye!');
    });
  }
  ...
  logon(credential) {
    ...
    return this.api.logon(credential.username, credential.password).catch(() => inquirer.prompt({
      type: 'confirm',
      name: 'tryagain',
  ...
  chooseAccountAndShowHistory(months) {
    return this.selectAccount().then(account => this.downloadHistoryAndShow(account, moment().subtract(months, 'months').format(moment.formats.default))).then(() => this.chooseAccountAndShowHistory(months));
  }
  ...
  downloadHistoryAndShow(account, from, to = moment().format(moment.formats.default)) {
    return this.downloadHistory(account, from, to).then(history => {
      const allTransactions = history.pendings.map(t => Object.assign({}, t, { pending: true })).concat(history.transactions);
